...
internal class HalfOpenStateTest {
    lateinit var cbc: CircuitBreakerContext
    lateinit var changeState : KFunction<*>

    @Test
    fun structure() {
        val halfOpenStateClass = Konsist
            .scopeFromProject(moduleName = "circuit-breaker")
            .classes()
            .withPackage("fr.uga.miage.m1.states")
            .withName("HalfOpenState")

        halfOpenStateClass.assertNotEmpty(
            strict = true,
            additionalMessage = "La classe [HalfOpenState] doit être déclarée dans le package [fr.uga.miage.m1.states]"
        )

        halfOpenStateClass
            .assertTrue(
                strict = true,
                additionalMessage = "La classe [HalfOpenState] doit hériter de la classe [${State::class.qualifiedName}]"
            ) {
                it.hasParentClassOf(State::class)
            }
        val loadWeatherFunction = halfOpenStateClass
            .functions()
    ...
    fun initialize_state() {
        ...
        val state: State = Class.forName(halfOpenStateClass.first().fullyQualifiedName)
            .kotlin
            .primaryConstructor
            ?.callBy(
                mapOf(
                    Class.forName(halfOpenStateClass.first().fullyQualifiedName).kotlin.primaryConstructor!!.parameters.first { it.name == "context" } to null
                )) as State
    ...
    @Test
    fun test_halfopen_to_open_if_connection_is_ko() {
        ...
        WeatherServiceImpl.setErrorStatus(true)
        cbc.state.loadWeather("France", "Grenoble")
        assertTrue(
            cbc.state.javaClass.getName().endsWith("OpenState"), "Bad states, current states must be .." + "OpenState"
        )
    }

    @Test
    fun test_halfopen_to_close_if_connection_is_ok() {
        // To initialize the cache if needed
        getWeather(cbc,"France", "Grenoble")
        val halfOpenStateClass = Konsist
            .scopeFromProject(moduleName = "circuit-breaker")
